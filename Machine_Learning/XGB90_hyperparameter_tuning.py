...
# # 設定arange(0.01,0.2,0.1)
# XGB = XGBClassifier(booster='gbtree',random_state=90)

# # 用网格搜索调整eta arange(0.01,0.2,0.01)
# param_grid = {'max_depth':np.arange(0.01,0.2,1)}
...
# # 設定arange(0.2,0.5,0.01)
# XGB = XGBClassifier(booster='gbtree',random_state=90)

# # 用网格搜索调整eta arange(0.2,0.5,0.01)
# param_grid = {'max_depth':np.arange(0.2,0.5,1)}
# GS = GridSearchCV(XGB, param_grid, cv=10)
# GS.fit(data, target)
...
# print(best_param, best_score)
# print(data.shape)
# #{'eta':0.45000000000000023}0.4399907977338863

# # 設定arange(0.01,0.3,0.01)
# XGB = XGBClassifier(booster='gbtree',random_state=90)

# # 用网格搜索调整eta arange(0.01,0.3,0.01)
# param_grid = {'max_depth':np.arange(0.01,0.3,1)}
# GS = GridSearchCV(XGB, param_grid, cv=10)
# GS.fit(data, target)
...
# # 設定arange(0.01,0.5,0.01)
# XGB = XGBClassifier(booster='gbtree',random_state=90)

# # 用网格搜索调整eta arange(0.01,0.5,0.01)
# param_grid = {'max_depth':np.arange(0.01,0.5,1)}
# GS = GridSearchCV(XGB, param_grid, cv=10)
# GS.fit(data, target)
...
# print(best_param, best_score)
# print(data.shape)
# #{'eta':0.12000000000001} 0.4270886686783303



# XGB = XGBClassifier(max_depth=33, eta=0.1, booster='gbtree',random_state=90)

# # 用网格搜索调整max_depth
# param_grid = {'max_depth':np.arange(30,60,1)}
# GS = GridSearchCV(XGB, param_grid, cv=10)
# GS.fit(data, target)
...
# print(best_param, best_score)
# print(data.shape)
# {'max_depth': 33} 0.4177734115092657


# XGB = XGBClassifier(eta=0.1, booster='gbtree',random_state=90)

# # 用网格搜索调整min_child_weight
# param_grid = {'min_child_weight':np.arange(1,30,1)}
# GS = GridSearchCV(XGB, param_grid, cv=10)
# GS.fit(data, target)
...
# print(best_param, best_score)
# print(data.shape)
# {'min_child_weight': 6} 0.42771972928146323
...
# filename = GS.predict_proba(test)   # 想要印出的程式碼 

# Result ="XGBClassifier(max_depth=33, eta=0.1, booster='gbtree',random_state=90).csv" # 印出CSV.名稱  
# def OutputCSV():   
